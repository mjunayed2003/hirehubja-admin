...
const ListSelect = ({ editor }) => {
  if (!editor) return null;
  const current = editor.isActive("bulletList")
    ? "bullet"
...
const Toolbar = ({ editor, isEditing, isSaving, onEdit, onCancel, onSave }) => (
  <div className="flex items-center gap-1 px-4 py-2 bg-white border-b border-gray-200 sticky top-0 z-10 flex-wrap shadow-sm">
    ...
    <div className="ml-auto flex items-center gap-2">
      {isEditing ? (
        <>
          <button type="button" onClick={onCancel} className="px-3 py-1.5 text-sm text-gray-600 bg-gray-100 hover:bg-gray-200 rounded-md transition-colors">
            Cancel
          </button>
...
// ─── Core PageEditor ──────────────────────────────────────────────────────────
const PageEditor = ({ pageName, defaultTitle }) => {
  ...
  // Backend returns { key, title, content } directly — no .data wrapper
  useEffect(() => {
    if (data && editor) {
      const t = data.title || defaultTitle;
  ...
  const handleSave = async () => {
    try {
      ...
      // updateContent returns { success, message, data: { key, title, content } }
            ...
            <>
              {isEditing ? (
                <input
                  ...
                  className="w-full text-2xl font-bold text-gray-900 mb-8 border-b-2 border-[#43B948] focus:outline-none bg-transparent pb-1"
                />
              ) : (
                <h1 className="text-2xl font-bold text-gray-900 mb-8 pb-2 border-b border-dashed border-gray-200">
                  {title}
                </h1>
              )}
